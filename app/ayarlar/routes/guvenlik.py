...
bp = Blueprint('guvenlik', __name__)


@bp.route('/guvenlik', methods=['GET', 'POST'])
@login_required
@role_required('admin')
def index():
    if request.method == 'POST':
        anahtarlar = ['oturum_suresi', 'min_sifre_uzunlugu', 'max_giris_denemesi']
        for anahtar in anahtarlar:
            deger = request.form.get(anahtar, '')
            SistemAyar.set(anahtar, deger, user_id=current_user.id)
        flash('Guvenlik ayarlari basariyla kaydedildi.', 'success')
        return redirect(url_for('ayarlar.guvenlik.index'))
